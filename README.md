...
- `PUBLIC_FORMSPREE_ENDPOINT`: complete Formspree form URL used to deliver contact enquiries to Nafis’s inbox.
...
The Formspree URL is stored as an Actions secret named `PUBLIC_FORMSPREE_ENDPOINT`; production builds require it. Add `PUBLIC_PLAUSIBLE_DOMAIN` as a repository variable when analytics is ready. The playlist ID is checked into the deployment workflow.
